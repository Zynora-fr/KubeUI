...
final class KubeUIStatusOverlayHudRenderer {
	...
	static void onRenderGui(RenderGuiEvent.Post event) {
		...
		for (var status : KubeUIStatusHud.active()) {
			...
			var item = BuiltInRegistries.ITEM.getOptional(ResourceLocation.tryParse(status.icon())).orElse(null);
			if (item != null) {
	...
	private static void drawRow(dev.kubeui.gui.GuiGraphicsExtractor graphics, net.minecraft.client.gui.Font font, int x, int y, int accentColor, ItemStack icon, String name, String durationText) {
