...
final class KubeUIMachineMenu extends AbstractContainerMenu {
	...
	KubeUIMachineMenu(int containerId, Inventory inventory, KubeUIMachineBlockEntity machine) {
		...
		// 1.21.1 has no addStandardInventorySlots(...) helper - inlined here matching the real
		// vanilla convention (3x9 main inventory grid, then a separate 9-wide hotbar row 58px below).
		...
		for (int col = 0; col < 9; col++) {
			addSlot(new Slot(inventory, col, 8 + col * 18, 106 + 58));
		}
	...
	public void clicked(int slotIndex, int button, ClickType type, Player clickingPlayer) {
		if (slotIndex >= MACHINE_SLOT_COUNT && slotIndex < TOP_SLOT_COUNT && type == ClickType.PICKUP
