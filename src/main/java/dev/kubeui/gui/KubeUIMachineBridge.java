		...
		if (entries.isEmpty()) {
			...
			for (var entry : entries) {
				if (entry instanceof CompoundTag tag) {
					String kind = tag.getStringOr("kind", "?");
					int progress = tag.getIntOr("progress", 0);
					int processTicks = Math.max(1, tag.getIntOr("processTicks", 1));
					int energy = tag.getIntOr("energy", 0);
					int maxEnergy = Math.max(1, tag.getIntOr("maxEnergy", 1));
					long crafted = tag.getLongOr("crafted", 0);
