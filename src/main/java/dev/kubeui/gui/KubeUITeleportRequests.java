...
final class KubeUITeleportRequests {
	...
	static String accept(ServerPlayer target) {
		...
		requester.teleportTo(targetLevel, target.getX(), target.getY(), target.getZ(), java.util.Set.of(), requester.getYRot(), requester.getXRot(), true);
		return requester.getGameProfile().name();
	}
